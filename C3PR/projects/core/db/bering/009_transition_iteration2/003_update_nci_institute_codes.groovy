class UpdateNciInstituteCodes extends edu.northwestern.bioinformatics.bering.Migration {
	void up() {
			...
			execute("update organizations set nci_institute_code='unknown or does not exist' where nci_institute_code='Warren'");
			execute("update csm_group set group_name='edu.duke.cabig.c3pr.domain.HealthcareSite.unknown or does not exist' where group_name='edu.duke.cabig.c3pr.domain.HealthcareSite.Warren'");
			execute("update csm_protection_element set protection_element_name='edu.duke.cabig.c3pr.domain.HealthcareSite.unknown or does not exist',object_id='edu.duke.cabig.c3pr.domain.HealthcareSite.unknown or does not exist' where protection_element_name='edu.duke.cabig.c3pr.domain.HealthcareSite.Warren'");
			execute("update csm_protection_group set  protection_group_name='edu.duke.cabig.c3pr.domain.HealthcareSite.unknown or does not exist' where protection_group_name='edu.duke.cabig.c3pr.domain.HealthcareSite.Warren'");
	}
