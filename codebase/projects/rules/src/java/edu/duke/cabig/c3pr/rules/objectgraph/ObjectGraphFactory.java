...
import gov.nih.nci.cabig.c3pr.rules.objectgraph.NavigationPath;
import gov.nih.nci.cabig.c3pr.rules.objectgraph.ObjectGraph;
...
public class ObjectGraphFactory {
    ...
    private ObjectGraphFactory() throws Exception {
        ...
        InputStream inputStream = Thread.currentThread().getContextClassLoader()
                        .getResourceAsStream("object-graph.xml");

        Unmarshaller unmarshaller = JAXBContext.newInstance(
                        "edu.duke.cabig.c3pr.rules.objectgraph").createUnmarshaller();
    ...
    public NavigationPath findNavigationPath(String sourceObjectType, String targetObjectType) {
        for (NavigationPath navigationPath : objectGraph.getNavigationPath()) {
            if (navigationPath.getSourceObjectType().equals(sourceObjectType)
                            && navigationPath.getTargetObjectType().equals(targetObjectType)) {
