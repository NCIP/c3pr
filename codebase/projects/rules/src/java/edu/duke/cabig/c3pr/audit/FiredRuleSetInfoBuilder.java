...
import edu.duke.cabig.c3pr.rules.common.CategoryConfiguration;
import edu.duke.cabig.c3pr.rules.common.RuleUtil;
...
public class FiredRuleSetInfoBuilder {
    ...
    public FiredRuleSetInfo build(String bindUri) {
        ...
        if (i < 0) {
            ...
            buildForC3pr(str1, firedRuleSetInfo);
        }

        return firedRuleSetInfo;
    }
    
    private void buildForC3pr(String partialUri, FiredRuleSetInfo firedRuleSetInfo) {

            int k = partialUri.indexOf(".");
            String str1 = partialUri.substring(k + 1, partialUri.length());
            String ruleSetName = str1;
            firedRuleSetInfo.setRuleSetName(ruleSetName);
            firedRuleSetInfo.setRuleSetType("Rules for C3PR");
    ...
    private void buildForSponsorDefinedStudyLevelRules(String partialUri,
                    FiredRuleSetInfo firedRuleSetInfo) {
    ...
    private void buildForInstitutionDefinedStudyLevelRules(String partialUri,
                    FiredRuleSetInfo firedRuleSetInfo) {
        int i = partialUri.indexOf(".");
        String useFullString = partialUri.substring(i + 1, partialUri.length());
        int a = useFullString.indexOf(".");
    ...
    public static void main(String[] args) {
         ...
         * Sponsor level rules
         */
        String sponsorLevel = RuleUtil.getPackageName(CategoryConfiguration.SPONSOR_BASE
                        .getPackagePrefix(), "NCI", "ASSES_AE_RULES");

        /**
         * Sponsor study level
         */

        String sponsorstudylevel = RuleUtil.getStudySponsorSpecificPackageName(
                        CategoryConfiguration.SPONSOR_DEFINED_STUDY_BASE.getPackagePrefix(),
                        "mytestStudy", "wake_forest", "Asses_AE_RULES");

        /**
         * Institution level
         */
        String institutionlevel = RuleUtil.getPackageName(CategoryConfiguration.INSTITUTION_BASE
                        .getPackagePrefix(), "Duke Cancer Center", "Report_ scheuling rules");

        /**
         * Institution study level
         */
        String institutionStudy = RuleUtil.getStudySponsorSpecificPackageName(
                        CategoryConfiguration.INSTITUTION_DEFINED_STUDY_BASE.getPackagePrefix(),
                        "yourteststudy", "Semanticbits", "Report Scheduling rules");
