...
import gov.nih.nci.cabig.c3pr.rules.brxml.MetaData;
 ...
 * <i>org.drools.repository.RulesRepository</i>.
 * <p>
 * In case we need to get more functionality from the repository than what drools provide we have
 * the flexibility to do so in this class.
 * 
...
public class RepositoryServiceImpl extends JcrDaoSupport implements RepositoryService {
    ...
    /*
     * This method creates a rule (non-Javadoc)
     * 
     * @see edu.duke.cabig.c3pr.rules.repository.RepositoryService#createRule(edu.duke.cabig.c3pr.rules.brxml.Rule)
     */
    public String createRule(Rule rule) throws RuleException{
        ...
        if (metaData.getCategory() != null && metaData.getCategory().size() > 0) {
            ...
            categoryName = (initialCategory != null) ? initialCategory.getPath() + "/"
                            + initialCategory.getMetaData().getName() : getDefaultCategory();
        }

        AssetItem asset = packageItem.addAsset(metaData.getName(), metaData.getDescription(),
                        categoryName, metaData.getFormat());
    ...
    public void updateRule(Rule rule) throws RuleException {
        ...
        if (meta.getDateEffective() != null) assetItem.updateDateEffective(meta.getDateEffective()
                        .toGregorianCalendar());

        if (meta.getDateExpired() != null) assetItem.updateDateExpired(meta.getDateExpired()
                        .toGregorianCalendar());
        ...
        for (int i = 0; i < numberOfCategories; i++) {
            categories[i] = categoryList.get(i).getPath() + "/"
                            + categoryList.get(i).getMetaData().getName();
        }
     ...
     * (non-Javadoc)
     * 
     * @see edu.duke.cabig.c3pr.rules.repository.RepositoryService#createRuleSet(edu.duke.cabig.c3pr.rules.brxml.RuleSet)
     */
    public String createRuleSet(RuleSet ruleSet) {
        PackageItem item = getRulesRepository().createPackage(ruleSet.getName(),
    ...
    /*
     * This method loads the package aka RuleSet from the repository (non-Javadoc)
     * 
     * @see edu.duke.cabig.c3pr.rules.repository.RepositoryService#getRuleSet(java.lang.String)
     */
    ...
    public void deleteRule(String ruleSetName, String ruleName) {
         ...
         * 
         * while (itr.hasNext()) { AssetItem a = (AssetItem) itr.next();
         * if(a.getUUID().equals(ruleName)) { r = a.getName(); break; } //a.remove();
    ...
    public RuleSetInfo getRegisteredRuleset(String bindUri) {
        ...
        try {
            ...
            ruleSetInfo.setContent(rulesRepositoryEx.convertByteArrayToObject(rulesRepositoryEx
                            .loadCompiledPackage(bindUri).getBinaryContentAsBytes()));
     ...
     * 
     */
    public List<Rule> getRulesByCategory(String categoryTag) throws RuleException{
        ...
        for (AssetItem assetItem : assetItems) {
            // This is a hack ... because the drools-repository doesn't treat versioning well. Lets
            // avoid duplicates for this release
    ...
    /**
     * This will be improved by using Reflection + Mapping. Dirty way of manual mapping now.
     */
    private void copyToMetaData(MetaData metaData, AssetItem assetItem) throws RuleException{
    ...
    /*
     * This method check for the existence of the package aka RuleSet (non-Javadoc)
     * 
     * @see edu.duke.cabig.c3pr.rules.repository.RepositoryService#containsRuleSet(java.lang.String)
     */
