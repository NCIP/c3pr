...
public class ObjectGraphBasedEditor extends PropertyEditorSupport {
        ...
        for (int i = 0; i < methodsToInvoke.size(); i++) {
            ...
            if (targetObject instanceof Collection) {
                ...
                while (it.hasNext()) {
                    ...
                    methodInvocater = CustomMethodInvocater.parse(element,
                                    getGetterString(defaultComparatorProperty));
                ...
                if (targetObject instanceof Collection) {
                    throw new Exception("No Object Found in Collection with identifier "
                                    + ids[idIndex] + " in object path " + objectPath);
                }
                idIndex++;
            }
        }
    ...
    public Object getObjectFromGraph() throws Exception {
        Object targetObject = commandObject;
        for (int i = 0; i < methodsToInvoke.size(); i++) {
            CustomMethodInvocater methodInvocater = CustomMethodInvocater.parse(targetObject,
                            methodsToInvoke.get(i));
            targetObject = methodInvocater.invoke();
        }
        return targetObject;
    }
    ...
    public void setMethodsToInvoke(String path) {
        ...
        if (logger.isDebugEnabled()) {
            logger
                            .debug("setMethodsToInvoke(String) - there are " + properties.length + " properties in " + path + ":"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
    ...
    private boolean equal(Object obj, String text) {
        if (defaultComparatorType.equalsIgnoreCase(DATA_TYPE_INT)) {
            if (((Integer) obj).intValue() == Integer.parseInt(text)) return true;
            else return false;
        }
        if (defaultComparatorType.equalsIgnoreCase(DATA_TYPE_STRING)) {
            if (((String) obj).equalsIgnoreCase(text)) return true;
            else return false;
        }
        return false;
    }
    ...
    public String toString() {
        ...
        sb.append("\n");
        sb.append("sample getter for property friends and argument John of type String:"
                        + getGetterString("friends", new String[] { "John" }) + "\n");
        return sb.toString();
    }
    ...
    public String getGetterString(String property, String[] args) {
        ...
        return "get" + new String("" + property.charAt(0)).toUpperCase()
                        + property.substring(1, property.length()) + "(" + argString + ")";
    }
}
