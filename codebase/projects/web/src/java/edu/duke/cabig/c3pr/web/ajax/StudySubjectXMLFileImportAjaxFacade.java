...
public class StudySubjectXMLFileImportAjaxFacade {
    ...
    private static Log log = LogFactory.getLog(StudySubjectXMLFileImportAjaxFacade.class);

    public String getTable(Map parameterMap, HttpServletRequest request,
                    Collection<StudySubject> studySubjects) throws Exception {
        ...
        return build(model, studySubjects, "Imported Registrations", action).toString();
    }

    protected Object build(TableModel model, Collection studySubjects, String title, String action)
                    throws Exception {
