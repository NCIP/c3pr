 ...
 */
public class ManageEpochTab<C extends StudySubjectWrapper> extends
		RegistrationTab<C> {

	public ManageEpochTab() {
		super("Manage Epochs", "Change Current Epoch",
				"registration/reg_manage_epochs");
	}
	...
	public ModelAndView getEpochSection(HttpServletRequest request,
		...
		map.put("epochType", epoch.getTreatmentIndicator() ? "Treatment"
				: "Non-Treatment");
		...
		if (studySubject.getCurrentScheduledEpoch().getEpoch().getId()
				.intValue() == epoch.getId().intValue())
			map.put("isCurrentScheduledEpoch", new Boolean(true));
		if (epoch.getEpochOrder() < studySubject.getCurrentScheduledEpoch()
				.getEpoch().getEpochOrder()) {
			map.put("notRegistrable", new Boolean(true));
		} else if (epoch.getEpochOrder() == studySubject
				.getCurrentScheduledEpoch().getEpoch().getEpochOrder()
				&& studySubject.getCurrentScheduledEpoch().getEpoch().getId() != epoch
						.getId()) {
			map.put("notRegistrable", new Boolean(true));
		}
