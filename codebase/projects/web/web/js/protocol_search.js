...
/*
  ...
  if (blnStatus)
  {
    if (checkConsolidator())
    {
      if(checkShipper())
      {
        window.location.replace('protocol_select.jsp');
...
*/
        window.location.replace('protocol_select.jsp');
        document.frmProtocolSearch.submit();
}

function sendToInput()
{
  document.frmProtocolSearch.action = "protocol_edit.jsp";
  document.frmProtocolSearch.submit();
}
