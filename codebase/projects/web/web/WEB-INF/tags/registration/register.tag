            ...
            <c:if test="${registration.studySite.study.randomizationType.name == 'PHONE_CALL'&& registration.scheduledEpoch.epoch.randomizedIndicator}">
				...
				<c:choose>
					<c:when test="${registration.studySite.study.blindedIndicator}">
						<td class="labelR"><fmt:message key="registration.enterKitNumber"/></td><td><input type="text" name="studySubject.scheduledEpoch.scheduledArms[0].kitNumber" id="kitNumber" size="20" class="required validate-notEmpty"/></td>
					</c:when>
					<c:otherwise>
		            ...
		            <c:if test="${childStudySubject.studySite.study.randomizationType.name == 'PHONE_CALL'&& childStudySubject.scheduledEpoch.epoch.randomizedIndicator}">
						...
						<c:choose>
							<c:when test="${childStudySubject.studySite.study.blindedIndicator}">
								<td class="labelR"><fmt:message key="registration.enterKitNumber"/></td><td><input type="text" name="studySubject.childStudySubjects[${status.index}].scheduledEpoch.scheduledArms[0].kitNumber" id="kitNumber" size="20" class="required validate-notEmpty"/></td>
							</c:when>
							<c:otherwise>
								<td class="labelR"><fmt:message key="registration.selectArm"/></td><td>
