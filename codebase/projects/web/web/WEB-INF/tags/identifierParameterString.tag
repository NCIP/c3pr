...
<c:when test="${identifier.class.name=='edu.duke.cabig.c3pr.domain.OrganizationAssignedIdentifier'}">
	...
	<c:set var="sourceType" value="organizationNciId"/>
	<c:set var="sourceValue" value="${identifier.healthcareSite.nciInstituteCode}"/>
</c:when>
<c:otherwise>
	<c:set var="assignedBy" value="system"/>
	<c:set var="sourceType" value="systemName"/>
	<c:set var="sourceValue" value="${identifier.systemName}"/>
</c:otherwise>
</c:choose>
assignedBy=${assignedBy }&${ sourceType}=${sourceValue }&identifierType=${identifier.type }&identifier=${identifier.value }
