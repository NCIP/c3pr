...
<c:forEach items="${flow.tabs}" var="atab">
    ...
    <li class="tab ${current ? 'current' : ''}">
        <img src="<tags:imageUrl name="tab2${current ? '_h' : ''}_L.gif"/>" width="1" height="16" align="absmiddle"><a href="#" class="tab${atab.number}"><c:if test="${empty showNumber}">${atab.number + 1}. </c:if>${atab.shortTitle}</a><img src="<tags:imageUrl name="tab2${current ? '_h' : ''}_R.gif"/>" width="6" height="16" align="absmiddle">
    </li>
</c:forEach>
</ul>
