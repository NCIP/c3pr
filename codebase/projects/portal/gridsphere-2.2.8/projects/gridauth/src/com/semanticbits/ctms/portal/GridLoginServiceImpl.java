...
public class GridLoginServiceImpl implements LoginService, PortletServiceProvider {


    private PortletLog log = SportletLog.getInstance(GridLoginServiceImpl.class);
    ...
    public User login(PortletRequest req)
        ...
        if (certificate == null) {
            ...
            //if no user is found check if grid authentication is needed
            if (user == null) {
                Collections.sort(modules);
                AuthenticationException authEx = null;

                Iterator it = modules.iterator();
                //check if grid login is highest priority
                LoginAuthModule mod = (LoginAuthModule) it.next();
                log.debug("using grid login");
                if (mod.isModuleActive() && mod.getModuleName().toLowerCase().contains("cagrid")) {

                    //create local user if it does not exist
                    User gridUser = new SportletUserImpl();
                    gridUser.setUserName(loginName);
                    gridUser.setUserID(loginName);
                    try {
                        //check auth
                        mod.checkAuthentication(gridUser, loginPassword);
                        userManagerService.saveUser(gridUser);
                        user = gridUser;
                        success = true;
                    } catch (AuthenticationException e) {
                        log.debug("Grid Authentication failed for user " + user.getUserName());
                    }
                }
            }
