...
public class CCTSCaXchangeBroadcasterTester extends MasqueradingDaoTestCase<StudySubjectDao> {
    ...
    protected void setUp() throws Exception {
        ...
        messageBroadcaster = (CCTSMessageBroadcaster) getApplicationContext().getBean("messageBroadcaster");
        marshaller = new XmlMarshaller((String) getApplicationContext().getBean("c3pr-xml-castorMapping"));
    }


    public void testCaXchangeMessageSender() throws Exception {
        for (StudySubject subject : getDao().getAll()) {
            messageBroadcaster.setNotificationHandler(cctsMessageWorkflowCallbackFactory.createWorkflowCallback(subject));
            String message = marshaller.toXML(subject);
            System.out.println(message);
            Document messageDOM = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new InputSource(new StringReader(message)));
