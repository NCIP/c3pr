...
public class SecurityContextTestUtils {
    ...
    /**
     * Switch to c3pr_admin user.
     * THis data is provisioned during migration
     */
    ...
    /**
     * Switch to a dummy user with no authority.
     * Use this to test if security interceptors
     * are working
     */
    ...
    private static void switchUser(String username){
        ...
        GrantedAuthority[] authorities = new GrantedAuthority[]{
                new GrantedAuthorityImpl("c3pr.user")};
        Authentication auth = new TestingAuthenticationToken(username,
                "does_not_matter", authorities);
