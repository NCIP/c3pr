...
@C3PRUseCases( { CREATE_STUDY, UPDATE_STUDY, SEARCH_STUDY,
		CREATE_STUDY_INVESTIGATOR, ADD_STRATIFICATION, ADD_DISEASE,
		VERIFY_SUBJECT })
public class StudyDaoTest extends DaoTestCase {
	...
	public ArrayList<StratumGroup> buildStratumGroupWithScac() {
		...
		stratumGroup.getStratificationCriterionAnswerCombination().addAll(
				scacList);
	...
	public void testSaveCalloutRandomizations() throws Exception {
		Integer savedId;
		{
			...
			((CalloutRandomization) cRandomization)
					.setCalloutUrl("trialUrl.com");
		...
		{
			Study loaded = dao.getById(savedId);
			CalloutRandomization cr = (CalloutRandomization) loaded
					.getTreatmentEpochs().get(0).getRandomization();
	...
	/*
	 * specifically created to test if stratum groups are saved with all the ans
	 * combinations
	 */
	public void testSaveStudyWithAnsComb() throws Exception {
		Integer savedId;
		{
			...
			scac1.setStratificationCriterionPermissibleAnswer(sc1
			...
			scac2.setStratificationCriterionPermissibleAnswer(sc1
					.getPermissibleAnswers().get(1));
			...
			scac3.setStratificationCriterionPermissibleAnswer(sc1
					.getPermissibleAnswers().get(2));
			...
			scac4.setStratificationCriterionPermissibleAnswer(sc2
					.getPermissibleAnswers().get(0));
			...
			scac5.setStratificationCriterionPermissibleAnswer(sc2
					.getPermissibleAnswers().get(1));
			...
			scac6.setStratificationCriterionPermissibleAnswer(sc2
					.getPermissibleAnswers().get(2));
		...
		{
			Study loaded = dao.getById(savedId);
			StratumGroup sg = ((TreatmentEpoch) loaded.getEpochs().get(0))
					.getStratumGroups().get(0);

			assertEquals(((TreatmentEpoch) loaded.getEpochs().get(0))
					.getStratumGroups().size(), 1);
			assertNotNull(sg.getStratificationCriterionAnswerCombination());
			assertEquals(sg.getStratificationCriterionAnswerCombination()
					.size(), 2);
	...
	public void testSaveStudyWithRandomizations() throws Exception {
		Integer savedId;
		{
			...
			// StratificationCriterion sc = new StratificationCriterion();
			// List <StratificationCriterion> scList = new ArrayList
		...
		{
			Study loaded = dao.getById(savedId);
			BookRandomization br = (BookRandomization) loaded
					.getTreatmentEpochs().get(0).getRandomization();
			int i = br.getBookRandomizationEntry().get(0).getStratumGroup()
					.getCurrentPosition();
			assertEquals(i, 1);
			// assertNotNull("Could not reload study with id " + savedId,
			// loaded);
			// assertEquals("Wrong name", "ShortTitleText",
			// loaded.getShortTitleText());
		}
	}
	...
	public void testComboGenerator() {
		...
		for (int i = 0; i < scSize; i++) {
			tempList = te.getStratificationCriteria().get(i)
					.getPermissibleAnswers();
			doubleArr[i] = new StratificationCriterionPermissibleAnswer[tempList
					.size()];
	...
	public ArrayList<StratumGroup> comboGenerator(TreatmentEpoch te,
			StratificationCriterionPermissibleAnswer[][] myArr,
			int intRecurseLevel, ArrayList<StratumGroup> sgList,
		...
		for (int i = 0; i < myArr[intRecurseLevel].length; i++) {
			strPositionVal
					.setStratificationCriterionPermissibleAnswer(myArr[intRecurseLevel][i]);
			strPositionVal.setStratificationCriterion(te
			...
			if (intRecurseLevel < myArr.length - 1) {
				sgList = comboGenerator(te, myArr, intRecurseLevel + 1, sgList,
						strMyLine);
			} else {
				StratumGroup sg = new StratumGroup();
				sg.getStratificationCriterionAnswerCombination().addAll(
						strMyLine);
				sgList.add(sg);
			}
		}
	...
	/*
	 * //working version with simple strings public String
	 * comboGenerator(String[][] myArr, int intRecurseLevel, String strResult,
	 * String strLine){ String strPositionVal = ""; String strMyLine = "";
	 * for(int i= 0; i< myArr[intRecurseLevel].length ; i++){ strPositionVal =
	 * myArr[intRecurseLevel][i]; if(strLine.length() > 0){ strMyLine = strLine
	 * +"/"+ strPositionVal; } else { strMyLine = strPositionVal; }
	 * 
	 * if(intRecurseLevel < myArr.length-1){ strResult = comboGenerator(myArr,
	 * intRecurseLevel + 1, strResult, strMyLine); } else { strResult =
	 * strResult + "\n" + strMyLine; } } return strResult; }
	 */
	...
	private Study createDefaultStudyWithDesign(Study study) {
		...
		// Study Investigator
		StudyInvestigator studyInvestigator = studySite.getStudyInvestigators()
				.get(studySite.getStudyInvestigators().size());
		studyInvestigator.setRoleCode("role");
	...
	public void testGetStudyFundingSponsors() throws Exception {
		...
		assertEquals("Wrong number of study funding sponsors", 1, sponsors
				.size());
	...
	/**
	 * Test for retrieving all study coordinating centers associated with this
	 * Study
	 * 
	 * @throws Exception
	 */
	public void testGetStudyCoordinatingCenters() throws Exception {
		Study study = dao.getById(1000);
		List<StudyCoordinatingCenter> centers = study
				.getStudyCoordinatingCenters();
		assertEquals("Wrong number of study coordinating centers", 1, centers
				.size());
	...
	public void testSaveNewStudyWithFundingSponsor() throws Exception {
		...
		interruptSession();
		{
			...
			assertEquals("Wrong name", "ShortTitleText", loaded
					.getShortTitleText());
			assertEquals("Wrong study funding sponsor",
					"National Cancer Institute", loaded
							.getStudyFundingSponsors().get(0)
	...
	public void testFailureSaveTwoIdentifiersWithSameOrgAndSameTypeInStudy()
			throws Exception {
	...
	public void testFailureSaveTwoStudyOrganizationsWithSameOrgAndSameType()
			throws Exception {
		Study loadedStudy = dao.getById(1000);
		HealthcareSite site = healthcareSitedao.getById(1000);
		StudySite studySite = new StudySite();
	...
	public void testFailureSaveTwoStudyFundingSponsorIdentifiersWithSameValue()
			throws Exception {
		Study loadedStudy1 = dao.getById(1002);
