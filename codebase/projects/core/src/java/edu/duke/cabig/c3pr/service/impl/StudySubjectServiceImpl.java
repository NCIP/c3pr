...
public class StudySubjectServiceImpl extends CCTSWorkflowServiceImpl
    ...
    private Participant buildParticipant(Participant participant) throws C3PRCodedException {
        if (participant.getIdentifiers() == null || participant.getIdentifiers().size() == 0) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.MISSING.SUBJECT_IDENTIFIER.CODE"));
        }
            ...
            if (organizationAssignedIdentifier.getType().equals(this.prtIdentifierTypeValueStr)) {
                ...
                if (paList.size() > 1) {
                    ...
                    if (temp.getFirstName().equals(participant.getFirstName())
                        ...
                        throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.INVALID.ANOTHER_SUBJECT_SAME_MRN.CODE")
                                , new String[]{organizationAssignedIdentifier.getValue()});
                    }
                }
            }
    ...
    private Study buildStudy(Study study) throws C3PRCodedException {
        if (study.getIdentifiers() == null || study.getIdentifiers().size() == 0) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.MISSING.STUDY_IDENTIFIER.CODE"));
        }
        ...
        if (identifier == null) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.MISSING.STUDY_COORDINATING_IDENTIFIER.CODE"));
        }
        if (studies == null) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.NOTFOUND.STUDY_WITH_IDENTIFIER.CODE")
        ...
        if (studies.size() > 1) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.MULTIPLE.STUDY_SAME_CO_IDENTIFIER.CODE")
                    , new String[]{identifier.getHealthcareSite().getNciInstituteCode(), this.identifierTypeValueStr});
        }
        if (studies.get(0).getCoordinatingCenterStudyStatus() != CoordinatingCenterStudyStatus.ACTIVE) {
            throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.STUDY_NOT_ACTIVE")
        ...
        for (StudySite temp : study.getStudySites()) {
            if (temp.getHealthcareSite().getNciInstituteCode().equals(studySite.getHealthcareSite().getNciInstituteCode())) {
                if (temp.getSiteStudyStatus() != SiteStudyStatus.ACTIVE) {
                    throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.STUDYSITE_NOT_ACTIVE")
                            , new String[]{temp.getHealthcareSite().getNciInstituteCode()});
                }
                return temp;
            }
        }
        throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.NOTFOUND.STUDYSITE_WITH_NCICODE.CODE")
    ...
    private Epoch buildEpoch(List<Epoch> epochs, ScheduledEpoch scheduledEpoch) throws C3PRCodedException {
        ...
        throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.NOTFOUND.EPOCH_NAME.CODE")
...
//            scheduledTreatmentEpoch.setEligibilityIndicator(scheduledTreatmentEpochSource.getEligibilityIndicator());
            ...
            if (epoch.getRequiresArm()) {
                if (scheduledTreatmentEpochSource.getScheduledArm() != null
                    ...
                    if (arm == null) {
                        throw this.exceptionHelper.getException(getCode("C3PR.EXCEPTION.REGISTRATION.NOTFOUND.ARM_NAME.CODE")
