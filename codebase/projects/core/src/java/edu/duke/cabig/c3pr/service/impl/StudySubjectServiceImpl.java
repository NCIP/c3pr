...
public class StudySubjectServiceImpl extends CCTSWorkflowServiceImpl implements StudySubjectService {
    ...
    public ScheduledEpochDataEntryStatus evaluateScheduledEpochDataEntryStatus(
        ...
        if (!studySubject.getIfTreatmentScheduledEpoch()) return ScheduledEpochDataEntryStatus.COMPLETE;
        ...
        if (scheduledTreatmentEpoch.getRequiresArm()
                        ...
                        && (scheduledTreatmentEpoch.getScheduledArm() == null || scheduledTreatmentEpoch
    ...
    private void manageSchEpochWorkFlow(StudySubject studySubject, boolean triggerMultisite,
        ...
        if (scheduledEpoch.getScEpochDataEntryStatus() == ScheduledEpochDataEntryStatus.COMPLETE
            ...
            if (this.requiresCoordinatingCenterApproval(studySubject)
                            && !isLocalSiteCoOrdinatingCenterForStudy(studySubject)) {
    ...
    private static boolean evaluateStratificationIndicator(StudySubject studySubject) {
        if (studySubject.getStratumGroupNumber() != null) return true;
        ScheduledTreatmentEpoch scheduledTreatmentEpoch = (ScheduledTreatmentEpoch) studySubject
                        .getScheduledEpoch();
    ...
    private boolean isLocalSiteCoOrdinatingCenterForStudy(StudySubject studySubject) {
        return studySubject.getStudySite().getStudy().getStudyCoordinatingCenters().get(0)
                        .getHealthcareSite().getNciInstituteCode()
                        .equals(getLocalInstanceNCICode());
    }

    private boolean isLocalinstanceStudySite(StudySubject studySubject) {
        return studySubject.getStudySite().getHealthcareSite().getNciInstituteCode().equals(
                        getLocalInstanceNCICode());
    }
