...
public class StudyServiceImpl extends CCTSWorkflowServiceImpl implements StudyService {
    ...
    public StudyDataEntryStatus evaluateTreatmentEpochDataEntryStatus(Study study)
                    throws C3PRCodedException {
    ...
    public StudyDataEntryStatus evaluateStratificationDataEntryStatus(Study study)
                    throws C3PRCodedException {
    ...
    public StudyDataEntryStatus evaluateEligibilityDataEntryStatus(Study study)
        ...
        // TODO //Disabled unless more information is obtained on managing the eligibility criteria
        // for all epochs
    ...
    public StudyDataEntryStatus evaluateAmendmentStatus(Study study) throws C3PRCodedException {
    ...
    public StudyDataEntryStatus evaluateDataEntryStatus(Study study) throws C3PRCodedException {
        ...
        if (evaluateEligibilityDataEntryStatus(study) != StudyDataEntryStatus.COMPLETE) return StudyDataEntryStatus.INCOMPLETE;
        if (study.hasStratification()) {
            if (evaluateStratificationDataEntryStatus(study) != StudyDataEntryStatus.COMPLETE) return StudyDataEntryStatus.INCOMPLETE;
        }
        if (study.getRandomizedIndicator()) {
        ...
        if (study.getTreatmentEpochs().size() > 0) {
            if (evaluateTreatmentEpochDataEntryStatus(study) != StudyDataEntryStatus.COMPLETE) return StudyDataEntryStatus.INCOMPLETE;
        }
    ...
    public SiteStudyStatus evaluateSiteStudyStatus(StudySite studySite) throws C3PRCodedException {
        ...
        if (studySite.getStudy().getCoordinatingCenterStudyStatus() == CoordinatingCenterStudyStatus.ACTIVE) {
            ...
            if (((studySite.getIrbApprovalDate()) == null)
                            ...
                            || (studySite.getIrbApprovalDate().before(calendar.getTime()))) {
    ...
    public Study setStatuses(Study study, CoordinatingCenterStudyStatus targetStatus)
                    throws C3PRCodedException {
    ...
    public boolean statusSettable(Study study,
        ...
        CoordinatingCenterStudyStatus currentStatus = study.getCoordinatingCenterStudyStatus();
    ...
    public Study setSiteStudyStatus(Study study, StudySite studySite, SiteStudyStatus status)
        ...
        if ((status == SiteStudyStatus.TEMPORARILY_CLOSED_TO_ACCRUAL)
            ...
            else studySite.setSiteStudyStatus(status);
        }
        else {
