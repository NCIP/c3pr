...
/**
 * FactoryBean that can be used to return a value from a property object based
 * on a key. This factory bean can be used in situations that you nead to assign
 * a bean property or constructor argument as a result of a properties object
 * lookup.
 * <p>There are more ussage patterns:<br>
 * <li>Setting the properties object as an bean reference and key as
 * separated properties</li>
 * <li>Setting the properties object as a bean name and key as separate
 * properties</li>
 ...
 * </p>
 * <p>Setting the properties object as an bean reference and key as
 ...
 * </p>
 * <p>Setting the properties object as a bean name and key as separate
 * properties:
 * <li>set <code>targetBeanName</code> (@see #setTargetBeanName(String))</li>
 * <li>set <code>key</code> (@see #setKey(String))</li>
 * </p>
 * <p>Setting the properties object as a bean name and key as one:
 * <li>set <code>propertyPath</code> (@see #setStaticName(String))</li>
 * </p>
 * <p>Setting the properties object as a bean name and key as bean name.
 * A simple usage is based on bean name.
 * <pre class="code">
 ...
 * @since July 24, 2006
 */
public class PropertiesGetterFactoryBean
    implements FactoryBean, BeanNameAware, InitializingBean, BeanFactoryAware {
    ...
    if (targetObject == null) {
      ...
      if (targetBeanName == null || targetBeanName.trim().equals("")) {
        throw new IllegalArgumentException(
          "TargetBeanName can not be null or empty."
  ...
  /**
   * Sets the bean name. <br/>
   * The bean name will be interpreted as propertyPath & key pattern if the
   * targetObject/key or propertyPath are not set.
  ...
  /**
   * Seths the default value to be returned if the properties object does not
   * contain a valye of the key.
   ...
   * @param targetObject The propertyPath object to set.
   */
  ...
  /**
   * Sets the propertyPath to be retrieved.<br/>
   * The value should follow the pattern beanName[key]
   * where bean name is the name of a bean in the context that maps to a
