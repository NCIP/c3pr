...
/**
 * <p> 
 *  This wrapper class projects only a specific type of elements in the contained list.
 *  <br />
 *  <br /><b>Eg:</b><br />Say a list consists of the following objects <br />
 *  [a1,b1,a2,a3,b2,c1,c2,b3,null,b4], where a1..n, b1..n , c1..n corresponds to objects
 *  of type A, B, C respectively. 
 *  <pre><code>
 *     List<Object> list = .....;//[a1,b1,a2,a3,b2,c1,c2,b3,null,b4]
 *     ProejectedList<B> bList = new ProjectedList<B>(B.class, list);
 *     System.out.println(bList);// prints [b1, b2, b3, null, b4]
 *     ProejectedList<C> cList = new ProjectedList<C>(C.class, list);
...
public class ProjectedList<E> implements List<E>{
	...
	/**
	 * Adds the element <code>E</code> aObject ohe specified index.
	 * If the index is &gt;= the size of the projected list, the <code>element</code>
	 * will be appended to the <code>target list</code>.
	 * If the index is &lt; the size of the projected list,the <code>element</code>
	 * will be inserted right after the <code>n-th (n = index)</code> occurence 
	 * of object of type <code>klass</code> in the <code>target list</code>.   
	 */
	...
	/** 
	 * Return the element of the projected list identified at position(<code>index</code>).
	 * Will loop through the <code>target list</code> and returns the <code>n-th(n = index)</code> element
