class AddCsmDataForCtep extends edu.northwestern.bioinformatics.bering.Migration {
    ...
    void up() {
    	execute("delete from csm_user_group_role_pg where group_id in (select group_id from csm_group where group_name='edu.duke.cabig.c3pr.domain.HealthcareSite.CTEP')")
    	execute("delete from csm_pg_pe where protection_element_id in (select protection_element_id from csm_protection_element where protection_element_name='edu.duke.cabig.c3pr.domain.HealthcareSite.CTEP')");
    	execute("delete from CSM_PROTECTION_GROUP where PROTECTION_GROUP_NAME='edu.duke.cabig.c3pr.domain.HealthcareSite.CTEP'");
	    execute("delete from csm_protection_element where protection_element_name='edu.duke.cabig.c3pr.domain.HealthcareSite.CTEP'");
