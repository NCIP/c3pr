class MiagrateParticipantsToStudySubjectDemographics extends edu.northwestern.bioinformatics.bering.Migration {
    void up() {
	 	...
	 	if (databaseMatches('oracle')) {
	 		execute("insert into stu_sub_demographics (id,version,grid_id,retired_indicator,first_name,last_name,middle_name,maiden_name,administrative_gender_code,birth_date,ethnic_group_code,marital_status_code,race_code,prt_id,add_id) (select stu_sub_demographics_ID_SEQ.NEXTVAL,0,grid_id,participants.retired_indicator,first_name,last_name,middle_name,maiden_name,administrative_gender_code,birth_date,ethnic_group_code,marital_status_code,race_code,prt_id,add_id from participants join study_subjects on participants.id= study_subjects.prt_id)");
	 	}
