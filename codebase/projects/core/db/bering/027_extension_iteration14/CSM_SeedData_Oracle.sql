...
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'system_administrator',	'Configures the technical system level properties and behavior of the applications (i.e. the password policy, email setup, ESB, etc).', -1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'business_administrator',	'Manages the domain related application wide properties and behavior (i.e. label names, reference data lists, etc)',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'person_and_organization_information_manager',	'Manages organizations and rosters / Creates and updates person info including contact info, degrees/certifications, rosters they?re associated with',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'data_importer',	'Identifies studies defined by Coordinating Center and imports as a consumer that data defined elsewhere',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'user_administrator',	'Ability to read system personnel (research staff and investigators) and create/manage user accounts/application roles, defines Custom Combination Roles', 		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_qa_manager',	'Updates core study info after saving / opening study.(e.g. PI, title, phase, epochs/arms, basic study design)  Read-only review of study calendar template / releases templates to participating sites', 		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_creator',	'Creates the core study info (e.g. PI, title, description, phase, epochs/arms & basic study design, etc.) NOTE:  some sites may want to combine the supplemental study info roles into this role', 		-1);
...
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_team_administrator',	'Connects study level people to study and internal staff to study, Assigns internal staff to protocol, determines study artifact accessiblity for staff (e.g. study calendar templates, CRFs, etc.)',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_site_participation_administrator',	'Connects participating sites to a protocol',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,  'ae_rule_and_report_manager',	'Creates, manages, imports AE rules / Creates, manages, imports AE report definitions',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_calendar_template_builder',	'Creates and updates study calendar templates', 		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'registration_qa_manager',	'Updates registration information (study subject ID, Date of consent) after enrollment. Can waive the eligibility criteria for certain study subjects.', 		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval, 'subject_manager',	'Defines patient to system (remaining subject data managed by other roles which are not defined)',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'study_subject_calendar_manager',	'Creates and updates a subject-specific study calendar based on a study calendar template', 		-1);
--INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'registrar',	'Accepts and approves/denies subject registration requests, requests subject registration on a particular study',		-1);
...
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'ae_expedited_report_reviewer',	'Read-only: reviews, provides comments, and routes expedited reports through the review workflow',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'ae_study_data_reviewer',	'Read-only: reviews, provides comments, and adverse event data through a review workflow',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'lab_impact_calendar_notifier',	'Creates a calendar notification for a potential lab-based treatment modification',		-1);
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval,	'lab_data_user',	'Enters, edits, and imports labs from LIMS, viewing labs, selecting and sending labs to CDMS and caAERS',		-1);
...
INSERT INTO csm_group(group_id, group_name, group_desc, application_id) VALUES (csm_group_group_id_seq.nextval, 'data_analyst',	'Read-only: searches for data, uses built-in analysis tools, exports data to third party tools',		-1);
