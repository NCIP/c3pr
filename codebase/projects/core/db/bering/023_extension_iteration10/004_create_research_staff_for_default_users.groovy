class CreateResearchStaffForDefaultUsers extends edu.northwestern.bioinformatics.bering.Migration {
    void up() {
    	if (databaseMatches('postgres')){
			...
			execute("update research_staff set hcs_id = (select max(organizations.id) from organizations, identifiers where organizations.id = identifiers.org_id and identifiers.value = (select value from configuration where prop like 'localNciInstituteCode')) where first_name like 'c3pr_admin'");
			...
			execute("update research_staff set hcs_id = (select max(organizations.id) from organizations, identifiers where organizations.id = identifiers.org_id and identifiers.value = (select value from configuration where prop like 'localNciInstituteCode')) where first_name like 'c3pr_registrar'");
			...
			execute("update research_staff set hcs_id = (select max(organizations.id) from organizations, identifiers where organizations.id = identifiers.org_id and identifiers.value = (select value from configuration where prop like 'localNciInstituteCode')) where first_name like 'c3pr_study_coordinator'");
			...
			execute("update research_staff set hcs_id = (select max(organizations.id) from organizations, identifiers where organizations.id = identifiers.org_id and identifiers.value = (select value from configuration where prop like 'localNciInstituteCode')) where assigned_identifier like 'cctsdemo1'");
