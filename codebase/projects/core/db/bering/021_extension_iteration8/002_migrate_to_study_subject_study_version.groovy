    ...
    void up() {
    	  if (databaseMatches('postgres')){
	    	...
	    	if (databaseMatches('oracle')){
				...
				execute("insert into study_subject_consents (id,version,retired_indicator,informed_consent_signed_date,study_subject_ver_id) (select id,version,'false',informed_consent_signed_date,id from study_subjects)");
				execute("UPDATE study_subject_consents SET consent_id =  (SELECT consents.id AS consent_id  FROM consents   JOIN study_versions  ON consents.stu_version_id = study_versions.id  JOIN study_site_versions  ON study_site_versions.stu_version_id = study_versions.id JOIN study_subjects  ON study_subjects.sto_id = study_site_versions.sto_id  JOIN study_subject_versions ON study_subject_versions.spa_id = study_subjects.id where study_versions.id IN (SELECT DISTINCT study_versions.id FROM study_versions,study_site_versions, study_subject_versions WHERE study_subject_versions.study_site_ver_id = study_site_versions.id AND study_site_versions.stu_version_id=study_versions.id ) AND study_subject_consents.study_subject_ver_id = study_subject_versions.id )");
