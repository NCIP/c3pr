...
<script type="text/javascript">
	 var editor_${path}=new Ajax.InPlaceEditor('${path}-id', document.URL,{validations:'${validations}', cancelLink:false, cancelButton:true, okText:'ok', cancelText:'cancel',
	 														 callback: function(form, value) {
	 														 		return '_asynchronous=true&_asyncMethodName=doInPlaceEdit&_ajaxInPlaceEditParam=${path}&_pathToGet=${pathToGet}&${path}=' + escape(value)+'&healthcareSite=${healthcareSite}&groupIndex=${groupIndex}';
	 														  	}
	 														  });
</script>